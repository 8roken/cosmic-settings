...
pub struct Manager {
    ...
    pub light: ThemeCustomizer,
    pub dark: ThemeCustomizer,
...
pub struct ThemeCustomizer {
    pub builder: (ThemeBuilder, Option<Config>),
    pub theme: (Theme, Option<Config>),
    pub accent_palette: Option<Vec<Srgba>>,
...
impl Manager {
    pub fn selected_customizer(&mut self) -> &mut ThemeCustomizer {
    ...
    pub fn apply_to_all_customizers<F>(&mut self, f: F)
    where
        F: Fn(&mut ThemeCustomizer),
    {
        f(&mut self.light);
        f(&mut self.dark);
    }
    ...
    pub fn theme(&self) -> &Theme {
        let customizer = if self.mode.0.is_dark {
            &self.dark
        } else {
            &self.light
        };
...
impl ThemeCustomizer {
    pub fn replace_builder(&mut self, builder: ThemeBuilder) {
        ...
        if let Some(config) = self.builder.1.as_ref() {
            self.builder.0.write_entry(config);
        }
    ...
    pub fn set_window_hint(&mut self, color: Option<Srgb>) -> Option<bool> {
        tracing::info!("Setting new window hint: {:?}", color);
    ...
    pub fn set_bg_color(&mut self, color: Option<Srgba>) -> Option<bool> {
        let config = self.builder.1.as_ref()?;

        self.builder.0.set_bg_color(config, color).ok()
    }

    pub fn set_primary_container_bg(&mut self, color: Option<Srgba>) -> Option<bool> {
        let config = self.builder.1.as_ref()?;

        self.builder.0.set_primary_container_bg(config, color).ok()
    }

    pub fn set_accent(&mut self, color: Option<Srgb>) -> Option<bool> {
        let config = self.builder.1.as_ref()?;

        self.builder.0.set_accent(config, color).ok()
    }

    pub fn set_text_tint(&mut self, color: Option<Srgb>) -> Option<bool> {
        let config = self.builder.1.as_ref()?;

        self.builder.0.set_text_tint(config, color).ok()
    }

    pub fn set_neutral_tint(&mut self, color: Option<Srgb>) -> Option<bool> {
        let config = self.builder.1.as_ref()?;
    ...
    pub fn set_spacing(&mut self, spacing: Spacing) -> Option<bool> {
        ...
        let result = self.builder.0.set_spacing(config, spacing).ok()?;
        self.theme
            .0
    ...
    pub fn set_corner_radii(&mut self, corner_radii: CornerRadii) -> Option<bool> {
        ...
        let result = self.builder.0.set_corner_radii(config, corner_radii).ok()?;
    ...
    pub fn set_gap_size(&mut self, gap: u32) -> Option<bool> {
        let config = self.builder.1.as_ref()?;
        let mut builder = &mut self.builder.0;
    ...
    pub fn set_active_hint(&mut self, active_hint: u32) -> Option<bool> {
        ...
        if active_hint > builder.gaps.1 {
            ...
            if builder.set_gaps(config, gaps).unwrap_or_default() {
                self.theme.0.set_active_hint(self.theme.1.as_ref()?, gaps.1);
            }
            changed_gap = true
        }
